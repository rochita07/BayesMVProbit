...
$ \xi_i = f_i \quad z_i  > 0 \quad$ where $\quad z_i = (z_{11i}, z_{12i},z_{21i}, z_{22i},z_{23i})^t \quad$ is a vector of length 5  and $f_i$  is a $5 \times 5$ matrix, whose first 2x2 diagonal matrix would from $z_{1i}$ and the second 3x3 matrix would be from $z_{2i}$, the rest of the elements would be zero.
